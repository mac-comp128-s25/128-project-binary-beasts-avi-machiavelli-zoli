...
public class Main {
    ...
    public void playerTurn(Scanner input) {
        ...
        int numResponse = playerResponse(2,
            "Choose your action! \n1. Attack \n2. Spell \nType the number of the action you would like to take");
        if (numResponse == 1) {
            int order = 1;
            List<Attack> attackList = player.getAttacks();
            for (Attack attack : attackList) {
                System.out.println(order + " " + attack.getName() + ": deals " + ((Attack) attack).getDamage()
                    + " damage, and has a " + ((Attack) attack).getChanceToHit() + " chance to hit");
                order++;
            }
            ...
            for (Spell spell : spellList) {
                System.out.println(order + " " + spell.getName() + ": costs " + ((Spell) spell).getManaCost()
                    + " mana, deals " + ((Spell) spell).getDamage() + " damage, and targets all enemies:"
                    + ((Spell) spell).getTargeting());
                order++;
            }
    ...
    public Deque<Character> generateEncounter(int enemyCount, double difficulty) {
        ...
        for (int i = 0; i < enemyCount; i++) {
            Character enemyCharacter = new Enemy("Enemy " + (i + 1), 10 * difficulty, difficulty, .1 * difficulty,
                10 - (difficulty / 2), difficulty / 5, difficulty);
    ...
    /**
     * Creates all the Skill objects. Adds starting attacks/spells to the player's lists, and adds all
     * other Skills to the associated possible lists.
     */
    ...
    /**
     * Runs the skill tree. Allows the player to add a designated amount of Skills, and allows them to
     * choose from ability types and the different Skills associated with that ability.
     * 
     * @param scanner
    ...
    public void addSkills(Scanner scanner, int abilityNum) {
        while (abilityNum > 0) {
            System.out.println(
                "You have " + player.getAttacks().size() + " attacks, and " + player.getSpells().size() + " spells.");
            System.out
                .println("You have " + abilityNum + " ability choices remaining. Choose the type of your ability!");
            int abilityOrder = 1;
